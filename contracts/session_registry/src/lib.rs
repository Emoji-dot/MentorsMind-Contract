...
impl SessionRegistry {
    ...
    /// Monitor mentor networks for market concentration.
    ///
    /// The platform backend pushes per-network session counts
    /// (`network_ids` / `network_session_counts` parallel arrays),
    /// the total sessions across the whole segment, and the current
    /// count of independent active mentors. Results are persisted and
    /// an event is emitted when a high-concentration state is detected.
    /// Returns the [`DecentralizationMonitoring`] result. Only callable
    /// by the platform backend.
    pub fn monitor_mentor_networks(
        env: Env,
        ...
        // Persist raw inputs for subsequent queries.
        env.storage()
            .persistent()
            .set(&DataKey::NetworkIds, &network_ids);
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::NetworkIds, TTL_THRESHOLD, TTL_BUMP);
        env.storage()
            .persistent()
            .set(&DataKey::SegmentTotalSessions, &total_sessions);
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::SegmentTotalSessions, TTL_THRESHOLD, TTL_BUMP);
        env.storage()
            .persistent()
            .set(&DataKey::IndependentMentorCount, &independent_mentor_count);
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::IndependentMentorCount, TTL_THRESHOLD, TTL_BUMP);
        env.storage()
            .persistent()
            .set(&DataKey::TotalActiveMentorCount, &total_active_mentors);
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::TotalActiveMentorCount, TTL_THRESHOLD, TTL_BUMP);

        // Store per-network session counts.
        for i in 0..network_ids.len().min(network_session_counts.len()) {
            ...
            env.storage()
                .persistent()
                .set(&DataKey::NetworkSessionCount(nid), &cnt);
        }

        // Compute concentration metrics.
        let monitoring =
            shared_detect_network_concentration(&network_session_counts, total_sessions);

        env.storage()
            .persistent()
            .set(&DataKey::DecentralizationMonitoringRecord, &monitoring);
        env.storage().persistent().extend_ttl(
            &DataKey::DecentralizationMonitoringRecord,
            TTL_THRESHOLD,
            TTL_BUMP,
        );

        // Also assess and cache competition protection.
        let barrier_count: u32 = env
            .storage()
            .persistent()
            .get(&DataKey::BarrierSignalCount)
            .unwrap_or(0);
        let competition = shared_assess_competition_barriers(
            &env,
        ...
        env.storage()
            .persistent()
            .set(&DataKey::CompetitionProtectionRecord, &competition);
        env.storage().persistent().extend_ttl(
            &DataKey::CompetitionProtectionRecord,
            TTL_THRESHOLD,
            TTL_BUMP,
        );

        // Trigger market protection intervention computation.
        let fairness: MarketFairness = env
            .storage()
            .persistent()
            .get(&DataKey::MarketFairnessRecord)
        ...
        let protection_record = shared_compute_market_protection_intervention(
            &env,
            &monitoring,
            &competition,
            &fairness,
        ...
        env.storage()
            .persistent()
            .set(&DataKey::MarketProtectionIntervention, &protection_record);
        env.storage().persistent().extend_ttl(
            &DataKey::MarketProtectionIntervention,
            TTL_THRESHOLD,
            TTL_BUMP,
        );

        if protection_record.intervene {
            env.storage()
                .persistent()
                .set(&DataKey::MarketControlActive, &true);
            env.storage().persistent().extend_ttl(
                &DataKey::MarketControlActive,
                TTL_THRESHOLD,
                TTL_BUMP,
            );
            env.events().publish(
                (symbol_short!("market"), Symbol::new(&env, "concentrated")),
                (monitoring.hhi_score, monitoring.dominant_share_bps),
            );
        }
    ...
    /// Protect market competition by identifying and responding to barriers
    /// against independent mentors.
    ///
    /// The platform backend reports the number of barrier signals detected
    /// (e.g., coordinated review bombing, price undercutting campaigns).
    /// The method combines the signal with the stored mentor counts, updates
    /// the cached [`CompetitionProtection`] record, and returns it. Emits
    /// an event when barriers are confirmed. Only callable by the platform
    /// backend.
    pub fn protect_market_competition(
        env: Env,
        ...
        // Persist the updated barrier signal count.
        env.storage()
            .persistent()
            .set(&DataKey::BarrierSignalCount, &barrier_signal_count);
        env.storage().persistent().extend_ttl(
            &DataKey::BarrierSignalCount,
            TTL_THRESHOLD,
            TTL_BUMP,
        );
        ...
        let independent_count: u32 = env
            .storage()
            .persistent()
            .get(&DataKey::IndependentMentorCount)
            .unwrap_or(0);
        let total_count: u32 = env
            .storage()
            .persistent()
            .get(&DataKey::TotalActiveMentorCount)
            .unwrap_or(0);
        ...
        if competition.barriers_detected {
            env.events().publish(
                (
                    symbol_short!("mktcomp"),
                    Symbol::new(&env, "barrier_detected"),
                ),
                (competition.independent_ratio_bps, barrier_signal_count),
            );
        }

        // Update the combined market protection record.
        let monitoring: DecentralizationMonitoring = env
            .storage()
            .persistent()
            .get(&DataKey::DecentralizationMonitoringRecord)
    ...
    /// Enforce pricing fairness across the market segment.
    ///
    /// The platform backend pushes a rolling window of price-change events
    /// (`price_timestamps` / `price_changes_bps` parallel arrays, sorted
    /// chronologically) so the contract can detect coordinated pricing moves.
    /// Results are persisted; an event is emitted when coordination is found.
    /// Returns the [`MarketFairness`] result. Only callable by the platform
    /// backend.
    pub fn ensure_pricing_fairness(
        env: Env,
        ...
        // Persist the rolling price windows (capped to MONITORING_LOG_CAP).
        let ts_len = price_timestamps.len().min(MONITORING_LOG_CAP);
        let mut ts_window: Vec<u64> = Vec::new(&env);
        for i in 0..ts_len {
            ts_window.push_back(price_timestamps.get(i).unwrap_or(0));
        }
        let chg_len = price_changes_bps.len().min(MONITORING_LOG_CAP);
        let mut chg_window: Vec<u32> = Vec::new(&env);
        for i in 0..chg_len {
            chg_window.push_back(price_changes_bps.get(i).unwrap_or(0));
        }

        env.storage()
            .persistent()
            .set(&DataKey::MarketPriceTimestamps, &ts_window);
        env.storage().persistent().extend_ttl(
            &DataKey::MarketPriceTimestamps,
            TTL_THRESHOLD,
            TTL_BUMP,
        );
        env.storage()
            .persistent()
            .set(&DataKey::MarketPriceChangesBps, &chg_window);
        env.storage().persistent().extend_ttl(
            &DataKey::MarketPriceChangesBps,
            TTL_THRESHOLD,
            TTL_BUMP,
        );

        let fairness = shared_detect_pricing_coordination(&ts_window, &chg_window);

        env.storage()
            .persistent()
            .set(&DataKey::MarketFairnessRecord, &fairness);
        env.storage().persistent().extend_ttl(
            &DataKey::MarketFairnessRecord,
            TTL_THRESHOLD,
            TTL_BUMP,
        );
        ...
        let competition: CompetitionProtection = env
            .storage()
            .persistent()
            .get(&DataKey::CompetitionProtectionRecord)
            .unwrap_or(CompetitionProtection {
                fair_access: true,
                independent_ratio_bps: 10_000,
                barriers_detected: false,
                risk_score: 0,
                reason: Symbol::new(&env, "unset"),
            });
        let protection_record = shared_compute_market_protection_intervention(
            &env,
            &monitoring,
        ...
        if fairness.coordination_detected {
            ...
            env.events().publish(
                (
                    symbol_short!("mktfair"),
                    Symbol::new(&env, "coord_detected"),
                ),
                (
                    fairness.suspicious_price_moves,
                    fairness.risk_score,
                ),
            );
        }
    ...
    /// Restore competitive balance after a market-control intervention
    /// cooldown has elapsed. Only callable by the platform backend.
    pub fn restore_competitive_balance(env: Env) {
        let backend = Self::require_backend(&env);
        backend.require_auth();
        ...
        let record: MarketProtectionRecord = env
            .storage()
            .persistent()
            .get(&DataKey::MarketProtectionIntervention)
            .expect("NoMarketProtectionInterventionOnRecord");

        if !is_market_restoration_eligible(&record, env.ledger().timestamp()) {
            panic!("MarketRestorationNotEligible");
        }

        env.storage()
            .persistent()
            .remove(&DataKey::MarketProtectionIntervention);
        env.storage()
            .persistent()
            .remove(&DataKey::MarketControlActive);

        env.events().publish(
            (symbol_short!("mktrst"), Symbol::new(&env, "restored")),
            env.ledger().timestamp(),
        );
    }

    /// Get the current market protection intervention record.
    pub fn get_market_protection_record(env: Env) -> MarketProtectionRecord {
        env.storage()
            .persistent()
            .get(&DataKey::MarketProtectionIntervention)
    ...
    /// Get the current decentralization monitoring result.
    pub fn get_decentralization_monitoring(env: Env) -> DecentralizationMonitoring {
        env.storage()
            .persistent()
            .get(&DataKey::DecentralizationMonitoringRecord)
            .unwrap_or(DecentralizationMonitoring {
    ...
    /// Get the current competition protection assessment.
    pub fn get_competition_protection(env: Env) -> CompetitionProtection {
        env.storage()
            .persistent()
            .get(&DataKey::CompetitionProtectionRecord)
            .unwrap_or(CompetitionProtection {
    ...
    /// Get the current market fairness assessment.
    pub fn get_market_fairness(env: Env) -> MarketFairness {
        env.storage()
            .persistent()
            .get(&DataKey::MarketFairnessRecord)
            .unwrap_or(MarketFairness {
    ...
    /// Provide algorithm transparency for a caller's scoring request.
    ///
    /// The backend supplies:
    /// - `requester`: the address querying for score transparency.
    /// - `factor_weights_bps`: disclosed scoring factor weights (parallel to
    ///   factor labels; sum ≤ 10_000).
    /// - `total_factors`: total factor count in the full ranking formula.
    /// - `manipulation_signal_count`: active manipulation signals in this window.
    /// - `requested_disclosure_bps`: fraction of formula the requester wants.
    ///
    /// The function:
    /// 1. Tracks the probe in the rolling log (reverse-engineering detection).
    /// 2. Scores probing behaviour and computes a balanced disclosure level.
    /// 3. Produces an [`AlgorithmTransparency`] assessment.
    /// 4. Computes and caches an [`AlgorithmProtectionRecord`].
    /// 5. Returns the [`TransparencyBalance`] — callers use `chosen_disclosure_bps`
    ///    to decide how much to disclose.
    ///
    /// Only callable by the platform backend.
    pub fn provide_algorithm_transparency(
        env: Env,
        requester: Address,
        ...
        // 1. Record this probe.
        let probe_key = DataKey::AlgoProbeLog(requester.clone());
        ...
        let now = env.ledger().timestamp();
        if probes.len() < MONITORING_LOG_CAP {
        ...
        // Track distinct input variations (incremented externally; read here).
        let var_key = DataKey::AlgoProbeVariations(requester.clone());
        ...
        let re_protection =
            shared_detect_reverse_engineering(&probes, variations);
        env.storage()
            .persistent()
            .set(&DataKey::AlgoProbeProtection(requester.clone()), &re_protection);
        env.storage().persistent().extend_ttl(
            &DataKey::AlgoProbeProtection(requester.clone()),
            TTL_THRESHOLD,
            TTL_BUMP,
        );

        // 3. Transparency assessment.
        let transparency = shared_assess_algo_transparency(
            &factor_weights_bps,
            total_factors,
        ...
        env.storage()
            .persistent()
            .set(&DataKey::AlgoTransparencyRecord, &transparency);
        env.storage().persistent().extend_ttl(
            &DataKey::AlgoTransparencyRecord,
            TTL_THRESHOLD,
            TTL_BUMP,
        );

        // 4. Transparency balance (how much to disclose).
        let balance = shared_compute_transparency_balance(
            &env,
            requested_disclosure_bps,
            re_protection.risk_score,
        );

        // 5. Algorithm protection intervention.
        let monitoring: AlgorithmMonitoringResult = env
            .storage()
            .persistent()
            .get(&DataKey::AlgoMonitoringResult)
        ...
        let algo_protection = shared_compute_algo_protection(
            &env,
            &transparency,
            &re_protection,
            &monitoring,
        ...
        env.storage()
            .persistent()
            .set(&DataKey::AlgoProtectionRecord, &algo_protection);
        env.storage().persistent().extend_ttl(
            &DataKey::AlgoProtectionRecord,
            TTL_THRESHOLD,
            TTL_BUMP,
        );
        if algo_protection.intervention_active {
            env.storage()
                .persistent()
                .set(&DataKey::AlgoProtectionActive, &true);
            env.storage().persistent().extend_ttl(
                &DataKey::AlgoProtectionActive,
                TTL_THRESHOLD,
                TTL_BUMP,
            );
            env.events().publish(
                (symbol_short!("algo"), Symbol::new(&env, "protection")),
    ...
    /// Protect recommendation systems from reverse engineering and gaming.
    ///
    /// The backend reports `distinct_variations` probes made by `requester`
    /// since the last call (used for systematic-variation detection) and the
    /// current `manipulation_signal_count`. The method:
    /// 1. Updates the probe-variation counter.
    /// 2. Re-scores the reverse-engineering risk.
    /// 3. Returns the updated [`ReverseEngineeringProtection`] result.
    ///
    /// Only callable by the platform backend.
    pub fn protect_recommendation_systems(
        env: Env,
        requester: Address,
        distinct_variations: u32,
        manipulation_signal_count: u32,
        ...
        env.storage()
            .persistent()
            .set(&var_key, &distinct_variations);
        env.storage()
            .persistent()
            .extend_ttl(&var_key, TTL_THRESHOLD, TTL_BUMP);

        // Fetch existing probe log.
        let probes: Vec<u64> = env
            .storage()
            .persistent()
            .get(&DataKey::AlgoProbeLog(requester.clone()))
            .unwrap_or_else(|| Vec::new(&env));

        let re_protection =
            shared_detect_reverse_engineering(&probes, distinct_variations);
        env.storage().persistent().set(
            &DataKey::AlgoProbeProtection(requester.clone()),
            &re_protection,
        );
        ...
        // Refresh the algorithm protection record if risk changed.
        let transparency: AlgorithmTransparency = env
            .storage()
            .persistent()
            .get(&DataKey::AlgoTransparencyRecord)
            .unwrap_or(AlgorithmTransparency {
                fair_operation: true,
                transparency_score: 80,
                disclosure_bps: 5_000,
                factor_count: 0,
                manipulation_detected: manipulation_signal_count > 0,
            });
        let monitoring: AlgorithmMonitoringResult = env
            .storage()
            .persistent()
        ...
        if algo_protection.intervention_active {
            ...
            env.events().publish(
                (symbol_short!("algo"), Symbol::new(&env, "re_blocked")),
                (requester, re_protection.risk_score),
            );
        }
    ...
    /// Monitor ranking fairness by tracking score deviations and gaming signals.
    ///
    /// The backend pushes `score_timestamps` / `score_deviations_bps`
    /// (parallel arrays, sorted chronologically) representing recent ranking
    /// score deviations from baseline, and `coordinated_actor_count` (number
    /// of accounts acting in coordinated fashion in the current window).
    ///
    /// The method:
    /// 1. Persists the rolling score-deviation window.
    /// 2. Runs ranking-algorithm monitoring.
    ...
    ///
    /// Only callable by the platform backend.
    pub fn monitor_ranking_fairness(
        env: Env,
        ...
        // Persist rolling windows (capped).
        let ts_len = score_timestamps.len().min(MONITORING_LOG_CAP);
        let mut ts_window: Vec<u64> = Vec::new(&env);
        for i in 0..ts_len {
            ts_window.push_back(score_timestamps.get(i).unwrap_or(0));
        }
        let dev_len = score_deviations_bps.len().min(MONITORING_LOG_CAP);
        ...
        env.storage()
            .persistent()
            .set(&DataKey::RankingScoreTimestamps, &ts_window);
        env.storage().persistent().extend_ttl(
            &DataKey::RankingScoreTimestamps,
            TTL_THRESHOLD,
            TTL_BUMP,
        );
        env.storage()
            .persistent()
            .set(&DataKey::RankingScoreDeviations, &dev_window);
        env.storage().persistent().extend_ttl(
            &DataKey::RankingScoreDeviations,
            TTL_THRESHOLD,
            TTL_BUMP,
        );
        env.storage()
            .persistent()
            .set(&DataKey::RankingCoordinatedActors, &coordinated_actor_count);
        env.storage().persistent().extend_ttl(
            &DataKey::RankingCoordinatedActors,
            TTL_THRESHOLD,
            TTL_BUMP,
        );
        ...
        // Monitoring.
        let monitoring =
            shared_monitor_ranking_algorithm(&ts_window, &dev_window, coordinated_actor_count);
        env.storage()
            .persistent()
            .set(&DataKey::AlgoMonitoringResult, &monitoring);
        env.storage().persistent().extend_ttl(
            &DataKey::AlgoMonitoringResult,
            TTL_THRESHOLD,
            TTL_BUMP,
        );
        ...
        // Use a default probe protection (no active probing context here).
        ...
        let audit = shared_audit_algo_transparency(
            &transparency,
            &re_protection,
            &balance,
            &monitoring,
        ...
        env.storage()
            .persistent()
            .set(&DataKey::AlgoTransparencyAudit, &audit);
        env.storage().persistent().extend_ttl(
            &DataKey::AlgoTransparencyAudit,
            TTL_THRESHOLD,
            TTL_BUMP,
        );
        ...
        if !monitoring.operating_normally {
            ...
            env.events().publish(
                (symbol_short!("algo"), Symbol::new(&env, "gaming_detected")),
                (monitoring.suspicious_deviations, monitoring.manipulation_risk_score),
            );
        }
    ...
    /// Restore fair algorithm operation after the protection cooldown has
    /// elapsed. Only callable by the platform backend.
    pub fn restore_algo_fair_operation(env: Env) {
        let backend = Self::require_backend(&env);
        backend.require_auth();
        ...
        let record: AlgorithmProtectionRecord = env
            .storage()
            .persistent()
            .get(&DataKey::AlgoProtectionRecord)
            .expect("NoAlgoProtectionRecord");

        if !is_algo_restoration_eligible(&record, env.ledger().timestamp()) {
            panic!("AlgoRestorationNotEligible");
        }

        env.storage()
            .persistent()
            .remove(&DataKey::AlgoProtectionRecord);
        env.storage()
            .persistent()
            .remove(&DataKey::AlgoProtectionActive);

        env.events().publish(
            (symbol_short!("algo"), Symbol::new(&env, "restored")),
            env.ledger().timestamp(),
        );
    }

    /// Get the current algorithm protection record.
    pub fn get_algo_protection_record(env: Env) -> AlgorithmProtectionRecord {
        env.storage()
            .persistent()
            .get(&DataKey::AlgoProtectionRecord)
    ...
    /// Get the current algorithm transparency assessment.
    pub fn get_algo_transparency(env: Env) -> AlgorithmTransparency {
        env.storage()
            .persistent()
            .get(&DataKey::AlgoTransparencyRecord)
            .unwrap_or(AlgorithmTransparency {
    ...
    /// Get the current transparency audit record.
    pub fn get_transparency_audit(env: Env) -> TransparencyAuditRecord {
        env.storage()
            .persistent()
            .get(&DataKey::AlgoTransparencyAudit)
